...
class FaceClientApp:
    ...
    def enroll_face(self):
        ...
        try:
            ...
            self.status_var.set(f"Enroll berhasil: {resp.json().get('name')}")
        except Exception as exc:
    ...
    def verify_once(self):
        ...
        try:
            resp = requests.post(f"{SERVER_URL}/recognitions/verify", json=payload, timeout=10)
            resp.raise_for_status()
            data = resp.json()
            if data.get("matched"):
                self.status_var.set(
                    f"{data.get('role')} cocok: {data.get('name')} | tindakan: {data.get('door_action')}"
                )
            else:
                self.status_var.set("Tidak cocok / ditolak")
        except Exception as exc:
            self.status_var.set(f"Verifikasi gagal: {exc}")
